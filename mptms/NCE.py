...
def NCE(source_label: np.ndarray, target_label: np.ndarray):
    ...
    # p_z = np.zeros((C_t, 1), dtype=float)
    # for i in range(len(unique)):
    #     p_z[unique[i], 0] = counts[i]

    # unique, counts = np.unique(source_label, return_counts=True)
    # p_z = np.zeros((1, C_s), dtype=float)
    # for i in range(len(unique)):
    #     p_z[0, unique[i]] = counts[i]
    ...
    return np.sum(np.log((joint / (p_z + 1e-20)) + 1e-20) * joint)
